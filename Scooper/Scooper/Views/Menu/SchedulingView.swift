        ...
        ZStack {
            ...
            VStack {
                ...
                ScrollView(.vertical) {
                    VStack {
                        ...
                        Button {
                            ...
                            Task {
                                do {
                                    try await vm.getScoopTeam(id: scoopTeamID)
                                } catch {
                                    print(error)
                                }
                            }
                ...
                Button {
                    ...
                    scoopTime = dateFormatter.string(from: self.date)
                    Task {
                        try await vm.sendScheduleScoopUpNotification(name: login.name ?? "")
                        try await vm.scoopRequest(id: self.id, date: scoopDate, time: scoopTime, note: text, student: student ?? "")
                    }
                } label: {
                    Text("Request")
                }
...
#Preview {
    SchedulingView(scoopTeamID: "863F4E258FA3")
}
